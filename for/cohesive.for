Module cohesive_mod
  ! Check for cohesive damage initiation and calculate the cohesive damage variable.
...
Contains
  ...
  Subroutine cohesive_damage(m, p, delta, Pen, delta_n_init, B, FI, evolve_fatigue, R, damage, dR)
    ...
    del    = SQRT(Macaulay(del_n)*del_n + del_s*del_s)  ! Combined normal and shear
    ...
    del0n = delta_initiation(m%YT, Pen(2))  ! mode I cohesive initiation disp.
    del0s = delta_initiation(SC, Pen_sh)    ! mode II cohesive initiation disp.

    ! Mode mixity
    beta = del_s*del_s + Macaulay(del_n)*del_n
    If (beta == zero) Then
      beta = one
    Else
    ...
    DamageEvolution: If (evolve_damage .OR. evolve_fatigue) Then
      ...
      delfn = delta_final(m%YT, m%GYT)  ! mode I cohesive final disp.
      delfs = delta_final(SC, m%GSL)    ! mode II cohesive final disp.
      ...
      If (evolve_damage) Then
        ...
        damage = damage_area_to_penalty(R, d0, df)  ! Penalty stiffness based cohesive damage variable
  ...
  Pure Function interface_displacement_jumps(delta, penalty, damage, penalty_interace, normal) result(delta_interface)
    ...
    ! Returns
    Double Precision :: delta_interface
    ...
    Return
  End Function interface_displacement_jumps
  
  
  Pure Function Macaulay(n)
  
    Double Precision, intent(IN) :: n
    Double Precision :: Macaulay
  ...
  Pure Function delta_initiation(strength, penalty)
    ! Returns the cohesive displacement-jump corresponding to damage initiation.
    ...
    ! Returns
    Double Precision :: delta_initiation
    ! -------------------------------------------------------------------- !
    
    delta_initiation = strength / penalty
    
    Return
  End Function delta_initiation
  
  
  Pure Function delta_final(strength, fracture_toughness)
    ! Returns the cohesive displacement-jump corresponding to final failure.
  
    ! Arguments
    Double Precision, intent(IN) :: strength
    Double Precision, intent(IN) :: fracture_toughness
    
    ! Returns
    Double Precision :: delta_final
    
    ! Locals
    Double Precision, parameter :: two=2.d0
    ! -------------------------------------------------------------------- !
    delta_final = two * fracture_toughness / strength
    
    Return
  End Function delta_final
  
  
  Pure Function damage_penalty_to_area(damage_penalty, delta_initiation, delta_final) result(damage_area)
  
    ! Arguments
    Double Precision, intent(IN) :: damage_penalty
    Double Precision, intent(IN) :: delta_initiation
    Double Precision, intent(IN) :: delta_final
    
    ! Returns
    Double Precision :: damage_area
    
    ! Locals
    Double Precision, parameter :: one=1.d0
    ! -------------------------------------------------------------------- !
    damage_area = damage_penalty / ((delta_final/delta_initiation) * (one - damage_penalty) + damage_penalty)
    
    Return
  End Function damage_penalty_to_area
  
  
  Pure Function damage_area_to_penalty(damage_area, delta_initiation, delta_final) result(damage_penalty)
  
    ! Arguments
    Double Precision, intent(IN) :: damage_area
    Double Precision, intent(IN) :: delta_initiation
    Double Precision, intent(IN) :: delta_final
    
    ! Returns
    Double Precision :: damage_penalty
    
    ! Locals
    Double Precision, parameter :: one=1.d0
    ! -------------------------------------------------------------------- !
    damage_penalty = damage_area/(damage_area + (delta_initiation/delta_final)*(one - damage_area))
    
    Return
  End Function damage_area_to_penalty
