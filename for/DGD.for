...
Contains
  ...
  Subroutine DGDInit(U, F, F_old, m, p, sv, ndir, nshr, DT, density_abq, Cauchy, enerIntern, enerInelas)
    ...
    Double Precision :: eps_total(ndir,ndir)                               ! Total strain (includes plastic strain)
    Double Precision :: eps_old(ndir, ndir)                                ! Strain last increment
    ...
    Double Precision :: phi12, phi13, phi
    ...
    Else  ! Compression in 1-dir
      ...
      If (m%fiberCompDamFKT12 .OR. m%fiberCompDamFKT13) Then
        ...
        ! Calculate the kinking failure criterion
        sv%rfC = MAX(kinking_failure_criterion(m, Cauchy, two*eps_total(1,2), sv%phi0_12, 2), &
          kinking_failure_criterion(m, Cauchy, two*eps_total(1,3), sv%phi0_13, 3))

        ! Only decompose element if the kinking criterion exceeds 1
        If (sv%rfC > one) Then
        ...
        ! Calculate the fiber breakage failure criterion
        phi = MAX(phi12, phi13)
        sv%fifbc = fiber_break_fi(m, Cauchy(1,1), phi)
  ...
  Subroutine DGDKinkband(U, F, F_old, m, p, sv, ndir, nshr, DT, density_abq, Cauchy, enerIntern, enerInelas)
    ...
    Equilibrium: Do ! Loop to determine the current Fkb(:,1)
      ...
      ! Calculate the angle that define the rotation of the fibers
      phi12 = ATAN(R_cr(2,1)/R_cr(1,1))
      phi13 = ATAN(R_cr(3,1)/R_cr(1,1))
      sv%gamma_12 = phi12 - sv%phi0_12
      sv%gamma_13 = phi13 - sv%phi0_13
      ...
      sv%rfC_temp = MAX(kinking_failure_criterion(m, Cauchy, two*eps_total(1,2), sv%phi0_12, 2), &
          kinking_failure_criterion(m, Cauchy, two*eps_total(1,3), sv%phi0_13, 3))
      ...
      DO I=1,3
        ...
        If (restarts .EQ. 1) Then
          ...
          Call ro_plasticity(m, 2, two*dEkb_dFkb1(1,2,I), Sign(one, (epskb(1,2) - epskb_old(1,2))), sv%Inel12c, dplas12, dinel12)
          dEkb_dFkb1(1,2,I) = dEkb_dFkb1(1,2,I) - dplas12/two
          dEkb_dFkb1(2,1,I) = dEkb_dFkb1(1,2,I)
          Call ro_plasticity(m, 3, two*dEkb_dFkb1(1,3,I), Sign(one, (epskb(1,3) - epskb_old(1,3))), sv%Inel13c, dplas13, dinel13)
